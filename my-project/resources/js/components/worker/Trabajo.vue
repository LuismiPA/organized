...
export default {
    ...
    created() {
        axios.get('/user/detalles').then(
        response=>{this.usuario=response.data;
            axios.get('/api/limpiezas/trabajos/' + this.usuario.id).then(response => this.trabajos = response.data);
        }
        );
